\title{Using shotgun metagenomic data for rRNA gene community analysis}
\author{}
...
PCR amplification and sequencing of small subunit (SSU) rRNA genes
from environmental sample DNA extractions is now standard procedure
for culture-free assessment of microbial community structure. However,
amplicon sequencing is limited to specific amplicons and may be biased
by the choice of primers and technical artifacts such as chimeras.  In
contrast, metagenomic shotgun sequencing does not depend on
gene-targeted primers or PCR amplification and thus is not affected by
primer bias or chimeras, respectively. However, detecting, recovering,
and analyzing SSU rRNA genes from shotgun data is challenging because
of the large number and short lengths of the reads. We deeloped a
framework to search for SSU rRNA gene fragments in shotgun data
followed by supervised classification, clustering and operational
taxonomy unit (OTU) based diversity analysis. Taxonomy profile
comparison of shotgun data and amplicon data from the same sample
shows a bias against Actinobacteria and Verrucomicrobia in the V6-V8
primer set and a bias against Actinobacteria in the V4 primer. This
pipeline also enabled {\em de novo} OTU based diversity analyses. In
application to rhizosphere soil samples, shotgun metagenomic data show
higher diversity than amplicon data, but the same grouping of samples
in ordination analyses. The HMM model can recover Bacteria, Archaea
and Eukaryota. Further, the analysis framework can be applied to LSU
rRNA providing additional confirmation of community structure.
\end{abstract}

\section{Introduction}

Microorganisms are distributed in all four domains in the tree of life
and are recognized for their key roles in ecosystem processes,
nutrient recycling and industrial processes, and in the health or
disease of higher organisms.
% @CTB - add citation
They are highly diverse at the genetic level
because of their long history and large habitat diversity
\cite{ward2002}. Many microbes have not been
cultured, which has become especially clear with the advent of
culture-free rRNA analysis of environmental samples.
% @CTB - add citation.
In particular, high-throughput sequencing of short hyper-variable
regions of a phylogenetic marker gene, ribosomal Small Subunit (SSU
rRNA) gene, provides access to the uncultured majority
(\cite{streit2004}). These high-throughput technologies (also called
amplicon sequencing because they involve PCR amplification of targeted
gene) have been used extensively in community structure surveys and
have expanded our investigation of microbial diversity in many
environments
\cite{huse2008,caporaso2012miseq}. Meanwhile, the growing databases
of SSU rRNA genes provide a rich ecological and phylogenetic context
for SSU rRNA gene based community structure surveys
\cite{rdp2009,silva2013}.
...
% @CTB -- the QIIME folk tell me this isn't true...?
...
% @CTB - this last bit (starting with ``There are two types...'') needs
% to be integrated into the text better.
